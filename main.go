...
func BastillePkgClean(response http.ResponseWriter, router *http.Request) {
    // bastille pkg TARGET clean -y
    vars := mux.Vars(router)
    target := vars["target"]
    out, err := exec.Command("bastille", "pkg", target, "clean", "-y").Output()

    if err != nil {
        log.Println(err)
    }

    command := "bastille pkg " + target + " clean -y"
    remoteaddr := router.Header.Get("X-Forwarded-For")
    LogApiRequest(remoteaddr, command, http.StatusOK)
...
func BastilleConsoleRedirect(response http.ResponseWriter, router *http.Request) {
    vars := mux.Vars(router)
    target := vars["target"]

    request_uri := router.Header.Get("X-Forwarded-Host")
    request_scheme := router.Header.Get("X-Forwarded-Proto")
    go BastilleConsole(response, router, target)
    ApiRedirect(response, router, request_scheme + "://" + request_uri + ":1789/ttyd/" + target)
}

func ApiRedirect(response http.ResponseWriter, router *http.Request, url string) {
    http.Redirect(response, router, url, http.StatusSeeOther)
}

func BastilleConsole(response http.ResponseWriter, router *http.Request, target string) {
    // bastille console
    cmd := exec.Command("ttyd", "-i", "lo0", "-o", "-b", "/ttyd/" + target, "bastille", "console", target)
    err := cmd.Run()
    ...
    command := "bastille console " + target
    remoteaddr := router.Header.Get("X-Forwarded-For")
    LogApiRequest(remoteaddr, command, http.StatusOK)
}

func RocinanteHelp(response http.ResponseWriter, router *http.Request) {
    // rocinante help
    out, err := exec.Command("rocinante", "help").Output()

    if err != nil {
        log.Println(err)
    }

    command := "rocinante help"
    remoteaddr := router.Header.Get("X-Forwarded-For")
    LogApiRequest(remoteaddr, command, http.StatusOK)
...
func RocinantePkgUpgrade(response http.ResponseWriter, router *http.Request) {
    // rocinante pkg upgrade -y
    out, err := exec.Command("rocinante", "pkg", "upgrade", "-y").Output()

    if err != nil {
        log.Println(err)
    }

    command := "rocinante pkg upgrade"
    remoteaddr := router.Header.Get("X-Forwarded-For")
    LogApiRequest(remoteaddr, command, http.StatusOK)
...
func RocinantePkgClean(response http.ResponseWriter, router *http.Request) {
    // rocinante pkg clean -y
    out, err := exec.Command("rocinante", "pkg", "clean", "-y").Output()

    if err != nil {
        log.Println(err)
    }

    command := "rocinante pkg clean -y"
    remoteaddr := router.Header.Get("X-Forwarded-For")
    LogApiRequest(remoteaddr, command, http.StatusOK)
...
func RocinanteUpdate(response http.ResponseWriter, router *http.Request) {
    // rocinante update
    out, err := exec.Command("rocinante", "update").Output()

    if err != nil {
        log.Println(err)
    }

    command := "rocinante update"
    remoteaddr := router.Header.Get("X-Forwarded-For")
    LogApiRequest(remoteaddr, command, http.StatusOK)
...
func main() {
    ...
    router.HandleFunc("/v1/bastille", BastilleHelp).Methods("GET")
    ...
    router.HandleFunc("/v1/bastille/console/{target}", BastilleConsoleRedirect).Methods("GET")
    ...
    router.HandleFunc("/v1/bastille/pkg/{target}/clean", BastillePkgClean).Methods("GET")
    ...
    router.HandleFunc("/v1/rocinante/help", RocinanteHelp).Methods("GET")
    router.HandleFunc("/v1/rocinante/pkg/upgrade", RocinantePkgUpgrade).Methods("GET")
    router.HandleFunc("/v1/rocinante/pkg/clean", RocinantePkgClean).Methods("GET")
    router.HandleFunc("/v1/rocinante/update", RocinanteUpdate).Methods("GET")
